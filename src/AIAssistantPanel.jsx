...
export default function AIAssistantPanel({
  ...
  return <aside className={`ai-assistant-panel side-panel ${open ? 'open' : ''}`} aria-label="ResumeForge AI workspace">
    ...
    <div className="ai-provider-strip"><span className={`connection-dot ${connection === 'connected' ? 'connected' : ''}`}/><div><strong>{provider.label}</strong><small>{config.model || 'Model not set'} · {connection === 'connected' ? 'Connection verified' : 'Not verified'}</small></div><button onClick={() => setTab('connect')}><KeyRound size={14}/>Configure</button></div>
    ...
    <div className="ai-panel-body">
      {tab === 'create' && <section className="ai-workflow">
        ...
        <Field label="Languages and additional details"><textarea rows="3" value={`${intake.languages}${intake.additionalDetails ? `\n${intake.additionalDetails}` : ''}`} onChange={event => setIntakeField('languages', event.target.value)} placeholder="Languages, volunteering, publications, availability…"/></Field>
      ...
      {tab === 'connect' && <section className="ai-workflow">
        <div className="ai-section-intro"><PlugZap size={19}/><div><h3>Connect an AI provider</h3><p>Credentials stay in memory for this tab and are never included in saved resumes or exports.</p></div></div>
        ...
        <RunButton onClick={onTestConnection} busy={busy} disabled={!config.model.trim() || (config.provider !== 'compatible' && !config.apiKey.trim())} icon={PlugZap}>Test connection</RunButton>
        {connection === 'connected' && <div className="connection-success"><Check size={15}/>Connection verified for this tab</div>}
        <div className="ai-security-note"><ShieldCheck size={17}/><div><strong>Important security boundary</strong><p>This portable file can call providers directly for personal use. Do not publish it with a key embedded. Resume data is sent only when you click an AI action.</p></div></div>
      </section>}
